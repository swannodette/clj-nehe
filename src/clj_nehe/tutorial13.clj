...
(def *width* 640)
(def *height* 480)
(def *fog-color* [0.5 0.5 0.5 1])
(def *fog-mode* [:exp :exp2 :linear])
...
(defn init [state]
  (app/title! "Nehe Tutorial 13")
  (app/vsync! false)
  (app/display-mode! *width* *height*)
  (enable :texture-2d)
  (shade-model :smooth)
  (clear-color 0.5 0.5 0.5 1)
  (fog :fog-mode (*fog-mode* 0))
  (fog :fog-color *fog-color*)
  (fog :fog-density 0.35)
  (hint :fog-hint :dont-care)
  (fog :fog-start 1)
  (fog :fog-end 5)
  (enable :fog)
  (clear-depth 1)
...
(defn key-press [key state]
  (condp = key
    ...
    "g"    (let [fog-filter (mod (inc (:fog-filter state)) 3)]
             (fog :fog-mode (*fog-mode* fog-filter))
             (assoc state :fog-filter fog-filter))
